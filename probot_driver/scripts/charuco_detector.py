...
# 注意：本节点直接输出 OpenCV 光学坐标系下的位姿
# 光学坐标系定义: X右, Y下, Z前（指向场景）
...
class CharucoDetector:
    def __init__(self):
        ...
        self.dict = aruco.getPredefinedDictionary(aruco.DICT_5X5_100)
        self.board = aruco.CharucoBoard((10, 7), 0.03, 0.021, self.dict)
        ...
        rospy.loginfo("ChArUco Detector 启动，等待图像...（输出光学坐标系）")
    ...
    def image_callback(self, msg):
        ...
        if self.frame_count % 30 == 0:
            rospy.loginfo("已收到 %d 帧图像, 内参状态: %s",
                          self.frame_count,
                          "OK" if self.cam_matrix is not None else "未收到")
        ...
        rospy.loginfo_throttle(2, "检测到 %d 个 ArUco 码: %s",
                               len(ids), ids.flatten().tolist())
        ...
        # 直接使用 OpenCV 光学坐标系下的 4x4 变换矩阵（不做坐标系转换）
        rot_matrix, _ = cv2.Rodrigues(rvec)
        optical_pose = np.eye(4)
        optical_pose[:3, :3] = rot_matrix
        optical_pose[0, 3] = tvec[0][0]
        optical_pose[1, 3] = tvec[1][0]
        optical_pose[2, 3] = tvec[2][0]

        # 直接从光学位姿提取四元数
        q = tf.transformations.quaternion_from_matrix(optical_pose)

        t = geometry_msgs.msg.TransformStamped()
        t.header.stamp    = rospy.Time.now()   # 使用当前时间，避免 TF 外推错误
        t.header.frame_id = "hikrobot_camera"
        t.child_frame_id  = "camera_marker"

        t.transform.translation.x = optical_pose[0, 3]
        t.transform.translation.y = optical_pose[1, 3]
        t.transform.translation.z = optical_pose[2, 3]
        ...
        rospy.loginfo_throttle(1, "TF 已发布(光学坐标系): camera_marker  xyz=[%.3f, %.3f, %.3f]m",
                               optical_pose[0, 3], optical_pose[1, 3], optical_pose[2, 3])
